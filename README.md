...
- **Structured output is unsupported and rejected.** The Copilot SDK exposes no
  ...
  no-op and is accepted on both surfaces in both modes; an unknown `type` is
  rejected as a typo. `text.verbosity` is still accepted as a no-op for Codex
  compatibility, and strict mode still rejects `text` for any other control.
- **`include` is narrow.** OpenAI's documented `include` values are accepted in
  permissive mode, but only `reasoning.encrypted_content` has any effect;
  undocumented values are rejected and strict mode rejects `include` entirely.
