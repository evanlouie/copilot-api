...
- **`strict: true` is enforced by the proxy, not by the model.** OpenAI
  implements strict tools with constrained decoding, so a strict call cannot be
  malformed. The Copilot SDK exposes no equivalent control, so this proxy
  provides the same guarantee at the other end: a strict tool's declared
  `parameters` are compiled at request time, and the arguments the model emits
  are validated against that schema before the tool call is handed to the
  client. A client that sets `strict: true` and skips its own validation never
  receives arguments this proxy could not verify. Two consequences follow. A
  strict tool whose schema cannot be compiled is a `400` at request time, rather
  than a guarantee that silently was never enforceable. And when the model emits
  non-conforming arguments the turn fails with an explicit error naming the
  tool, instead of delivering the call anyway: this proxy does not own the
  model's decoding loop, so it cannot retry on the client's behalf. `strict` on
  a freeform `custom` tool is a `400`, because such a tool declares its input
  with a grammar in `format` rather than with a JSON Schema, so there is nothing
