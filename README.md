...
Current SDK target: `github.com/github/copilot-sdk/go v1.0.0-beta.10`. As of the
latest locally available module tags, a stable Go `v1.0.0` tag is not available
yet.
...
| Streaming                       | SSE streams are OpenAI-shaped. Chat streams forward SDK text deltas, buffer tool calls, emit complete tool-call deltas, and terminate with `[DONE]`. Responses SSE streams emit lifecycle events such as `response.created`, `response.in_progress`, `response.output_text.delta`, `response.output_text.done`, function-call argument events, `response.completed`, or `response.failed`, then `[DONE]`. Responses WebSocket mode emits JSON events and terminates failures with a top-level `error` for client compatibility.                                                                                                |
