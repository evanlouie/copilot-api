...
| Unsupported fields              | Validation follows one rule: reject unknown, accept-and-ignore known-but-unsupported, never silently drop something the client could have meant. Strict compatibility defaults to disabled, so harmless unsupported client knobs such as `temperature` are ignored. Parameters that mainstream clients set by default — `max_tokens`, `max_completion_tokens`, `max_output_tokens`, `parallel_tool_calls:false`, and forcing `tool_choice` values — are accepted and reported at debug level instead of failing the request. For Codex CLI compatibility, Responses `include` values, `text.verbosity`, and `text.format` are accepted as no-ops in permissive mode. Values OpenAI does not define (an unknown `include` entry, `text.format.type`, `tool_choice`, or Responses tool `type`) are rejected, as are unsupported semantics that would mislead clients; both fail closed with OpenAI-shaped `invalid_request_error` responses. |
...
- **Structured text output is unsupported.** `text.verbosity` is accepted as a
  no-op for Codex compatibility. `text.format` is shape-checked and accepted in
  permissive mode (an unknown `type` is rejected) but not enforced, and strict
  mode rejects `text` outright. Chat Completions `response_format` is still
  rejected because there is no equivalent "ignored controls" contract there.
