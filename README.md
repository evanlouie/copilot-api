...
- **`tool_choice` supports only model-default planning and no-tools mode.**
  ...
  because the Copilot SDK/runtime does not expose OpenAI-compatible enforcement.
  Rejecting them is not an option: OpenAI's own Structured Outputs guidance and
  several major SDKs send them by default.
