...
- **`strict: true` is enforced by the proxy, not by the model, and is
  ...
  unenforced**, not rejected: the tool is still offered to the model and its
  arguments pass through unvalidated. This happens for a freeform `custom` tool
  (it declares its input with a grammar in `format` rather than a JSON Schema,
  so there is nothing client-declared to constrain), for any external `$ref`
  (schemas are resolved with no loader), and for Draft-07 spellings such as
  boolean `exclusiveMinimum` that the Draft 2020-12 compiler cannot model.
  Rejecting these was tried and reverted: they are schemas real OpenAI accepts,
  sent by default by the very clients that set `strict: true` (Vercel AI SDK,
  LangChain/LangGraph, OpenAI Agents SDK, Cline), so a `400` broke working
  integrations over a guarantee that is best-effort here regardless. Each
  unenforceable tool is reported to the operator with its reason at **warn**
  level (`accepted strict: true but cannot enforce it`), but the client still
  sees a normal `200`. A client that sets `strict: true` should therefore keep
  validating arguments itself.
